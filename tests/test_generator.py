...
from geninit import Config, plan
from geninit.errors import (
    AnalysisError,
    CollisionError,
    ConfigurationError,
    OwnershipError,
...
def test_generation_preserves_content_outside_the_managed_block(tmp_path: Path) -> None:
    ...
    prefix, managed_and_suffix = after.split("# <geninit>", maxsplit=1)
    _, suffix = managed_and_suffix.split("# </geninit>\n", maxsplit=1)
...
def test_generated_initializer_passes_ruff(tmp_path: Path) -> None:
    ...
    assert (
        ...
        .startswith(
            '"""Automatically generated by geninit.\n\n'
            "Content between the geninit markers will be overwritten; keep both "
            'markers intact.\n"""\n'
        )
    )
